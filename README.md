...
This subfolder contains the materials for the <em>collatio</em> of the <em>Lucidario</em>, a work in progress for a critical edition in preparation. The Word files are distributed in folders with correlative numbers for each chapter. Each of these contains a 'Marcados' subfolder (documents for each witness containing the normalized text WITH punctuation and page foliation, and a color marking: red for duplicated passages or mistakes identified by the scribes but not cancelled, orange for Latin and <em>lacunae</em>, and blue for corrections by hands different to the main scribe or scribes) and a 'Limpios' subfolder (documents without punctuation but with the aforementioned color code, except the duplicated passages). The rubrics have been extracted from each chapter and placed in a separate document in the main chapter folder, titled 'Rubrica + Chapter Number'. This document also contains the definite title for the chapter. Also in the main folder, you will find two plain text files called 'Chapter Number+_separado' and 'Chapter Number+_unido' , which are the collation files for [ChryscoCollate](https://cental.uclouvain.be/chrysocollate/), containing the two collation files (one with all readings in separate, individual columns, and the other with shared readings grouped across witnesses).

Note that not all chapters have a collation file. They will be added regularly as the collation progresses.
